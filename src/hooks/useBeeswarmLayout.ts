...
export function useBeeswarmLayout(input: BeeswarmInput[], width: number) {
  ...
  useEffect(() => {
    ...
    const sim = forceSimulation(next)
      ...
      .alpha(1)
      .alphaMin(0.004)
      .alphaDecay(0.055) // ~1.5s of settling, easing out naturally
      .velocityDecay(0.34)
      .stop();
    ...
    const step = () => {
      sim.tick();
      // Hard band clamp: guarantees a row's swarm never bleeds into its
      // neighbour, however dense the field gets.
      for (const n of next) {
        const lo = n.yMin + n.r;
        const hi = n.yMax - n.r;
        if (n.y < lo) {
